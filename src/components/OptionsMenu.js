...
export default function OptionsMenu() {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-cave-dark">
      <div className="bg-cave-mid/90 backdrop-blur border-2 border-cave-light rounded-xl p-6 md:p-8 w-[90%] max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="font-pixel text-white/70 text-[10px] block mb-1">
              🎮 Button-Größe: {buttonSize}px
            </label>
            <input type="range" min={40} max={96} step={4} value={buttonSize}
              onChange={(e) => setButtonSize(Number(e.target.value))} className="w-full" />
            <div className="flex justify-between mt-1">
              <span className="font-pixel text-[7px] text-white/30">Klein</span>
              <span className="font-pixel text-[7px] text-white/30">Groß</span>
            </div>
            {/* Preview */}
            <div className="flex justify-center mt-2 gap-2">
              {['👊', '🛡️', '⬆️'].map((icon, i) => (
                <div key={i} className="rounded-full border border-white/20 flex items-center justify-center bg-white/5"
                  style={{ width: buttonSize, height: buttonSize, fontSize: Math.round(buttonSize * 0.38) }}>
                  {icon}
                </div>
              ))}
            </div>
          ...
          {/* Language */}
          <div>
            <label className="font-pixel text-white/70 text-[10px] block mb-2">{t('language')}</label>
