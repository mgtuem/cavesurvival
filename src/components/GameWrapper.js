  ...
  useEffect(() => {
    ...
    const initPhaser = async () => {
      ...
      game.scene.add('GameScene', scene, true, {
        ...
        multiplayerMgr: null, // plug in multiplayer manager here
      });
        ...
        <>
          ...
          {gameCtx.isDead && (
            <DeathOverlay
              ...
              onDecline={() => gameCtx.returnToMenu()}
            />
          )}
          ...
          {gameCtx.waveText && (
            <div className="absolute inset-0 z-40 flex items-center justify-center pointer-events-none">
              <div className="font-pixel text-cave-gold text-xl md:text-3xl animate-pulse drop-shadow-lg">
                {gameCtx.waveText}
              </div>
            </div>
          )}
          ...
          {gameCtx.easterEgg && (
            <div className="absolute inset-0 z-40 flex items-end justify-center pb-28 pointer-events-none">
              <div className="font-pixel text-[10px] md:text-sm text-cave-gold bg-black/60 px-5 py-3 rounded-lg border border-cave-gold/30 animate-float">
                {gameCtx.easterEgg}
              </div>
            </div>
          )}
